...
impl<T> SpinLock<T> {
    ...
    pub fn lock(&self) -> &mut T {
        // TODO
        while let Err(_) = self.locked.compare_exchange(false, true, Ordering::Release, Ordering::Acquire) {
            core::hint::spin_loop();
        }
    ...
    pub fn try_lock(&self) -> Option<&mut T> {
        ...
        if let Err(_) = self.locked.compare_exchange(false, true, Ordering::Release, Ordering::Acquire) {
            return None;
        }
