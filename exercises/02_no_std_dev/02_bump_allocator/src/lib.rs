...
unsafe impl GlobalAlloc for BumpAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        loop {
            ...
            match end {
                x if x > self.heap_end => return null_mut(),
                x => {
                    // Check if another thread also allocates
                    match self.next.compare_exchange(offset, x, Ordering::AcqRel, Ordering::Acquire) {
                        Ok(_) => break aligned_offset as *mut u8,
                        Err(_) => continue
                    };
                }
            }
